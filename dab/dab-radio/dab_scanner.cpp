...
DabScanner::DabScanner(RaonTunerInput* tuner, DabScannerObserver* observer)
    : m_tuner{tuner}, m_scanner_observer{observer} {
...
DabScanner::~DabScanner() { std::cout << "Destroying Scanner" << std::endl; };
  ...
  for (auto freq : DAB_FREQUENCIES) {
    ...
    for (auto const& x : m_listed_services) {
      m_scanner_observer->serviceFound(
          freq, m_ensemble_label,
          FICDecoder::ConvertLabelToUTF8(x.second.label, nullptr),
          x.second.audio_service.subchid, x.second.audio_service.dab_plus,
