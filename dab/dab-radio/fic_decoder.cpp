...
void FICDecoder::Process(const uint8_t* data, size_t len) {
  ...
  if (len % 32) {
    fprintf(
        stderr,
        "FICDecoder: Ignoring non-integer FIB count FIC data with %zu bytes\n",
        len);
    return;
  }

  for (size_t i = 0; i < len; i += 32) ProcessFIB(data + i);
}
  ...
  for (size_t offset = 0; offset < 30 && data[offset] != 0xFF;) {
    ...
    switch (type) {
      ...
      case 1:
        ...
        break;
        //		default:
        //			fprintf(stderr, "FICDecoder: received
        //unsupported FIG %d with %zu bytes\n", type, len);
    }
    offset += len;
  }
}
...
void FICDecoder::ProcessFIG0(const uint8_t* data, size_t len) {
  ...
  switch (header.extension) {
    ...
    case 19:
      ...
      break;
      //	default:
      //		fprintf(stderr, "FICDecoder: received unsupported FIG
      //0/%d with %zu field bytes\n", header.extension, len);
  }
}
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (!sc.IsNone()) {
      ...
      if (current_sc != sc) {
        ...
        fprintf(stderr,
                "FICDecoder: SubChId %2d: start %3zu CUs, size %3zu CUs, PL "
                "%-7s = %3d kBit/s\n",
          ...
          if (!ca) {
            switch (ascty) {
              ...
              case 63:  // DAB+
                ...
                AUDIO_SERVICE& current_audio_service =
                    service.audio_comps[subchid];
                if (current_audio_service != audio_service ||
                    ps != (service.pri_comp_subchid == subchid)) {
                  current_audio_service = audio_service;
                  if (ps) service.pri_comp_subchid = subchid;

                  fprintf(stderr,
                          "FICDecoder: SId 0x%04X: audio service (SubChId %2d, "
                          "%-4s, %s)\n",
      ...
      if (!msc_fic_flag) {
        ...
        if (current_sc.language != language) {
          ...
          fprintf(stderr, "FICDecoder: SubChId %2d: language '%s'\n", subchid,
                  ConvertLanguageToString(language).c_str());
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (ls_flag) {
      ...
      if (!msc_fic_flag) {
        ...
        FIC_SERVICE& service = GetService(sid);
        bool new_comp =
            service.comp_defs.find(scids) == service.comp_defs.end();
        int& current_subchid = service.comp_defs[scids];
        if (new_comp || current_subchid != subchid) {
          current_subchid = subchid;

          fprintf(stderr,
                  "FICDecoder: SId 0x%04X, SCIdS %2d: MSC service component "
                  "(SubChId %2d)\n",
                  sid, scids, subchid);
...
void FICDecoder::ProcessFIG0_9(const uint8_t* data, size_t len) {
  // FIG 0/9 - Time and country identifier - Country, LTO and International
  // table ensemble ECC/LTO and international table ID only
  ...
  if (ensemble != new_ensemble) {
    ...
    fprintf(stderr,
            "FICDecoder: ECC: 0x%02X, LTO: %s, international table ID: 0x%02X "
            "(%s)\n",
            ensemble.ecc, ConvertLTOToString(ensemble.lto).c_str(),
            ensemble.inter_table_id,
            ConvertInterTableIDToString(ensemble.inter_table_id).c_str());
    ...
    for (const fic_services_t::value_type& service : services) {
      ...
      if (s.pty_static != FIC_SERVICE::pty_none ||
          s.pty_dynamic != FIC_SERVICE::pty_none)
        UpdateService(s);
    }
  }
}
    ...
    for (size_t scids_ua = 0; scids_ua < num_scids_uas; scids_ua++) {
      ...
      if (ua_type == 0x002) {
        ...
        if (service.comp_sls_uas.find(scids) == service.comp_sls_uas.end()) {
          ...
          fprintf(stderr,
                  "FICDecoder: SId 0x%04X, SCIdS %2d: Slideshow (%zu bytes UA "
                  "data)\n",
...
void FICDecoder::ProcessFIG0_17(const uint8_t* data, size_t len) {
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (current_pty != pty) {
      // suppress message, if dynamic FIC messages disabled and dynamic PTY not
      // initally be set
      bool show_msg =
          !(disable_dyn_msgs && sd && current_pty != FIC_SERVICE::pty_none);
      ...
      if (show_msg) {
        ...
        fprintf(stderr, "FICDecoder: SId 0x%04X: programme type (%s): '%s'\n",
                ...
                ConvertPTYToString(pty, 0x01).c_str());
      }
...
void FICDecoder::ProcessFIG0_18(const uint8_t* data, size_t len) {
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (current_asu_flags != asu_flags || current_cids != cids) {
      ...
      fprintf(stderr,
              "FICDecoder: SId 0x%04X: ASu flags 0x%04X, cluster(s) %s\n", sid,
              asu_flags, cids_str.c_str());
...
void FICDecoder::ProcessFIG0_19(const uint8_t* data, size_t len) {
  ...
  for (size_t offset = 0; offset < len;) {
    ...
    if (current_ac != ac) {
      ...
      if (!disable_dyn_msgs) {
        fprintf(stderr,
                "FICDecoder: ASw cluster 0x%02X: flags 0x%04X, SubChId %2d\n",
      ...
      for (const fic_services_t::value_type& service : services) {
        const FIC_SERVICE& s = service.second;
        if (s.cids.find(cid) != s.cids.cend()) UpdateService(s);
      }
    }
  }
}
...
void FICDecoder::ProcessFIG1(const uint8_t* data, size_t len) {
  ...
  if (len != len_calced) {
    fprintf(stderr,
            "FICDecoder: received FIG 1/%d having %zu field bytes (expected: "
            "%zu)\n",
...
void FICDecoder::ProcessFIG1_0(uint16_t eid, const FIC_LABEL& label) {
  if (ensemble.eid != eid || ensemble.label != label) {
    ...
    std::string short_label_str =
        DeriveShortLabelUTF8(label_str, label.short_label_mask);
    fprintf(stderr,
            "FICDecoder: EId 0x%04X: ensemble label '"
            "\x1B[32m"
            "%s"
            "\x1B[0m"
            "' ('"
...
void FICDecoder::ProcessFIG1_1(uint16_t sid, const FIC_LABEL& label) {
  ...
  if (service.label != label) {
    ...
    std::string short_label_str =
        DeriveShortLabelUTF8(label_str, label.short_label_mask);
    fprintf(stderr,
            "FICDecoder: SId 0x%04X: programme service label '"
            "\x1B[32m"
            "%s"
            "\x1B[0m"
            "' ('"
...
void FICDecoder::ProcessFIG1_4(uint16_t sid, int scids,
                               const FIC_LABEL& label) {
  // programme services only
  ...
  if (comp_label != label) {
    ...
    std::string short_label_str =
        DeriveShortLabelUTF8(label_str, label.short_label_mask);
    fprintf(stderr,
            "FICDecoder: SId 0x%04X, SCIdS %2d: service component label '"
            "\x1B[32m"
            "%s"
            "\x1B[0m"
            "' ('"
...
void FICDecoder::UpdateSubchannel(int subchid) {
  ...
  for (const fic_services_t::value_type& service : services) {
    const FIC_SERVICE& s = service.second;
    if (s.audio_comps.find(subchid) != s.audio_comps.end()) UpdateService(s);
  }
}
...
void FICDecoder::UpdateService(const FIC_SERVICE& service) {
  ...
  if (service.HasNoPriCompSubchid() || service.label.IsNone()) return;
  ...
  for (const comp_defs_t::value_type& comp_def : service.comp_defs) {
    if (comp_def.second == service.pri_comp_subchid ||
        service.audio_comps.find(comp_def.second) == service.audio_comps.end())
      continue;
...
void FICDecoder::UpdateListedService(const FIC_SERVICE& service, int scids,
                                     bool multi_comps) {
  ...
  if (scids == LISTED_SERVICE::scids_none) {  // primary component
    ...
    if (cl_it != service.comp_labels.end()) ls.label = cl_it->second;
  }

  // use sub-channel information, if available
  fic_subchannels_t::const_iterator sc_it =
      subchannels.find(ls.audio_service.subchid);
  if (sc_it != subchannels.end()) ls.subchannel = sc_it->second;
  ...
  if (sls_scids != LISTED_SERVICE::scids_none &&
      service.comp_sls_uas.find(sls_scids) != service.comp_sls_uas.end())
...
std::string FICDecoder::ConvertLabelToUTF8(const FIC_LABEL& label,
  ...
  std::string result = CharsetTools::ConvertTextToUTF8(
      label.label, sizeof(label.label), label.charset, false, charset_name);

  // discard trailing spaces
  size_t last_pos = result.find_last_not_of(' ');
  if (last_pos != std::string::npos) result.resize(last_pos + 1);

  return result;
}

const size_t FICDecoder::uep_sizes[] = {
    16,  21,  24,  29,  35,  24,  29,  35,  42,  52,  29,  35,  42,
    52,  32,  42,  48,  58,  70,  40,  52,  58,  70,  84,  48,  58,
    70,  84,  104, 58,  70,  84,  104, 64,  84,  96,  116, 140, 80,
    104, 116, 140, 168, 96,  116, 140, 168, 208, 116, 140, 168, 208,
    232, 128, 168, 192, 232, 280, 160, 208, 280, 192, 280, 416};
const int FICDecoder::uep_pls[] = {
    5, 4, 3, 2, 1, 5, 4, 3, 2, 1, 5, 4, 3, 2, 5, 4, 3, 2, 1, 5, 4, 3,
    2, 1, 5, 4, 3, 2, 1, 5, 4, 3, 2, 5, 4, 3, 2, 1, 5, 4, 3, 2, 1, 5,
    4, 3, 2, 1, 5, 4, 3, 2, 1, 5, 4, 3, 2, 1, 5, 4, 2, 5, 3, 1};
const int FICDecoder::uep_bitrates[] = {
    32,  32,  32,  32,  32,  48,  48,  48,  48,  48,  56,  56,  56,
    56,  64,  64,  64,  64,  64,  80,  80,  80,  80,  80,  96,  96,
    96,  96,  96,  112, 112, 112, 112, 128, 128, 128, 128, 128, 160,
    160, 160, 160, 160, 192, 192, 192, 192, 192, 224, 224, 224, 224,
    224, 256, 256, 256, 256, 256, 320, 320, 320, 384, 384, 384};
const int FICDecoder::eep_a_size_factors[] = {12, 8, 6, 4};
const int FICDecoder::eep_b_size_factors[] = {27, 21, 18, 15};

const char* FICDecoder::languages_0x00_to_0x2B[] = {"unknown/not applicable",
                                                    "Albanian",
                                                    "Breton",
                                                    "Catalan",
                                                    "Croatian",
...
const char* FICDecoder::languages_0x7F_downto_0x45[] = {
    "Amharic",   "Arabic",      "Armenian",     "Assamese",    "Azerbaijani",
    "Bambora",   "Belorussian", "Bengali",      "Bulgarian",   "Burmese",
    "Chinese",   "Chuvash",     "Dari",         "Fulani",      "Georgian",
    "Greek",     "Gujurati",    "Gurani",       "Hausa",       "Hebrew",
    "Hindi",     "Indonesian",  "Japanese",     "Kannada",     "Kazakh",
    "Khmer",     "Korean",      "Laotian",      "Macedonian",  "Malagasay",
    "Malaysian", "Moldavian",   "Marathi",      "Ndebele",     "Nepali",
    "Oriya",     "Papiamento",  "Persian",      "Punjabi",     "Pushtu",
    "Quechua",   "Russian",     "Rusyn",        "Serbo-Croat", "Shona",
    "Sinhalese", "Somali",      "Sranan Tongo", "Swahili",     "Tadzhik",
    "Tamil",     "Tatar",       "Telugu",       "Thai",        "Ukranian",
    "Urdu",      "Uzbek",       "Vietnamese",   "Zulu"};

const char* FICDecoder::ptys_rds_0x00_to_0x1D[] = {"No programme type",
                                                   "News",
                                                   "Current Affairs",
                                                   "Information",
                                                   "Sport",
                                                   ...
                                                   "Folk Music",
                                                   "Documentary"};
const char* FICDecoder::ptys_rbds_0x00_to_0x1D[] = {"No program type",
                                                    "News",
                                                    "Information",
                                                    "Sports",
                                                    "Talk",
...
const char* FICDecoder::asu_types_0_to_10[] = {"Alarm",
                                               "Road Traffic flash",
                                               "Transport flash",
                                               "Warning/Service",
                                               "News flash",
...
std::string FICDecoder::ConvertLTOToString(const int value) {
  char lto_string[7];
  snprintf(lto_string, sizeof(lto_string), "%+03d:%02d", value / 2,
           (value % 2) ? 30 : 0);
  return lto_string;
}
...
std::string FICDecoder::ConvertDateTimeToString(FIC_DAB_DT utc_dt,
                                                const int lto, bool output_ms) {
  const char* weekdays[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};

  // if desired, apply LTO
  if (lto) utc_dt.dt.tm_min += lto * 30;
...
std::string FICDecoder::ConvertPTYToString(const int value,
                                           const int inter_table_id) {
...
std::string FICDecoder::ConvertASuTypeToString(const int value) {
  if (value >= 0 && value <= 10) return asu_types_0_to_10[value];
  return "unknown (" + std::to_string(value) + ")";
}

std::string FICDecoder::DeriveShortLabelUTF8(const std::string& long_label,
                                             uint16_t short_label_mask) {
  std::string short_label;

  for (size_t i = 0; i < long_label.length();
       i++)  // consider discarded trailing spaces
