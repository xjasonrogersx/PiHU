...
class StringTools {
 ...
 public:
  static string_vector_t SplitString(const std::string& s,
...
class CharsetTools {
 private:
  ...
  static std::string ConvertStringIconvToUTF8(
      const std::vector<uint8_t>& cleaned_data, std::string* charset_name,
      const std::string& src_charset);

 public:
  static std::string ConvertTextToUTF8(const uint8_t* data, size_t len,
...
class BitReader {
 ...
 public:
  BitReader(const uint8_t* data, size_t data_bytes)
      : data(data), data_bytes(data_bytes), data_bits(0) {}
...
struct AUDIO_SERVICE {
  ...
  AUDIO_SERVICE(int subchid, bool dab_plus)
      : subchid(subchid), dab_plus(dab_plus) {}

  bool operator==(const AUDIO_SERVICE& audio_service) const {
    return subchid == audio_service.subchid &&
           dab_plus == audio_service.dab_plus;
  }
  bool operator!=(const AUDIO_SERVICE& audio_service) const {
