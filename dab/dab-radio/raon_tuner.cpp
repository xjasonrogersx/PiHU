...
RaonTunerInput::RaonTunerInput() {
  ...
  if (m_usbDevice == NULL) throw std::runtime_error("Cannot open tuner");
...
void RaonTunerInput::switchPage(RaonTunerInput::REGISTER_PAGE regPage) {
  std::vector<uint8_t> switchData{0x21, 0x00, 0x00,
                                  0x02, 0x03, static_cast<uint8_t>(regPage)};
...
void RaonTunerInput::configurePowerType() {
  ...
  REG2F =
      static_cast<uint8_t>(REG2F | 0x10); /*PDDCDC_I2C = 1, PDLDO12_I2C = 0 ;*/
...
void RaonTunerInput::configureAddClock() {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (RD15 > 0) {
      ...
      std::cerr << LOG_TAG << " ADCClock SYNTH 1st step  UnLock..."
                << std::endl;
    }
  }
}
...
void RaonTunerInput::tdmbInitComm() {
  ...
  setRegister(0xA6, 0x30);  // 0x30 ==>NO TSOUT@Error packet, 0x10 ==> NULL PID
                            // PACKET@Error packet
...
void RaonTunerInput::rtvOEMConfigureInterrupt() {
  ...
  setRegister(0x28, 0x01); /* [5:3]INT1 out sel [2:0] INI0 out sel - 0:Toggle
                              1:Level,, 2:"0", 3:"1"*/

  setRegister(0x2A, 0x13); /* [5]INT1 pol [4]INT0 pol - 0:Active High, 1:Active
                              Low [3:0] Period = (INT_TIME+1)/8MHz*/
}
...
void RaonTunerInput::rtvRFInitilize() {
  ...
  short checkVal =
      (short)(((readRegister(0x10) << 8) | readRegister(0x11)) & 0xFFFF);
...
void RaonTunerInput::setFrequency(uint32_t frequencyKhz) {
  ...
  int nIdx =
      (freqMhz - DAB_CH_BAND3_START_FREQ_KHz) / DAB_CH_BAND3_STEP_FREQ_KHz;
...
void RaonTunerInput::readData() {
  ...
  if (ficInt) {
    ...
    // std::cerr << LOG_TAG << "FicData Hdr: " << +reFicRet.size() << " : " <<
    // +bytesTransfered  << " : " << std::hex << std::setfill('0') <<
    // std::setw(2) << +reFicRet[0] << +reFicRet[1] << +reFicRet[2] <<
    // +reFicRet[3] << std::dec << std::endl; std::cerr << LOG_TAG << "FicData:
    // " << std::hex << std::setfill('0') << std::setw(2) << +reFicRet[5] <<
    // +reFicRet[6] << +reFicRet[7] << +reFicRet[8] << std::dec << std::endl;
  ...
  if (msc1Overrun || msc1Underrun) {
    // std::cerr << LOG_TAG << "Clearing MSC memory for OverUnderRun, MSC0Int"
    // << +msc0Int << ", MSC0Overrun: " << +msc0Overrun << ", MSC0Underrun: " <<
    // +msc0Underrun << std::endl;
  ...
  if (msc1Int) {
    ...
    if (m_msc_observer && bytesTransfered >= 4) {
      ...
      //  audio->componentMscDataInput(std::vector<uint8_t>(mscRecBuff.begin()+4,
      //  mscRecBuff.begin()+bytesTransfered), false);
      m_msc_observer->mscData(std::vector<uint8_t>(
          mscRecBuff.begin() + 4, mscRecBuff.begin() + bytesTransfered));
    } else {
...
void RaonTunerInput::openSubChannel(uint8_t subchanId) {
  std::cerr << LOG_TAG << "Opening subchannel: " << std::hex << +subchanId
            << std::dec << std::endl;
...
uint8_t RaonTunerInput::getLockStatus() {
  ...
  std::cerr << LOG_TAG << "LockState at: " << +m_currentFrequency << " : "
            << +lockSt << std::endl;
...
void RaonTunerInput::getAntennaLevel() {
  ...
  if (lock_stat) {
    ...
    cer_period_cnt =
        (rcnt3 << 24) | (rcnt2 << 16) | (rcnt1 << 8) | rcnt0;  // 442368
...
int RaonTunerInput::bulkTransferData(uint8_t endPointAddress,
                                     std::vector<uint8_t>& buffer) {
  int actual;
  libusb_bulk_transfer(m_usbDevice, endPointAddress, buffer.data(),
