...
// --- StringTools
// -----------------------------------------------------------------
string_vector_t StringTools::SplitString(const std::string& s,
...
std::string StringTools::UTF8Substr(const std::string& s, size_t pos,
                                    size_t count) {
  std::string result = s;
...
std::string CharsetTools::ConvertCharEBUToUTF8(const uint8_t value) {
  // convert via LUT
  if (value <= 0x1F) return ebu_values_0x00_to_0x1F[value];
  if (value >= 0x7B) return ebu_values_0x7B_to_0xFF[value - 0x7B];
...
std::string CharsetTools::ConvertStringIconvToUTF8(
    const std::vector<uint8_t>& cleaned_data, std::string* charset_name,
  ...
  if (charset_name) *charset_name = src_charset;
  return std::string(output_bytes, output_len_orig - output_len);
}

std::string CharsetTools::ConvertTextToUTF8(const uint8_t* data, size_t len,
                                            int charset, bool mot,
  ...
  if (charset == 0b0000) {  // EBU Latin based
    if (charset_name) *charset_name = "EBU Latin based";

    std::string result;
    for (const uint8_t& c : cleaned_data) result += ConvertCharEBUToUTF8(c);
    return result;
  }
  ...
  if (charset == 0b1111) {  // UTF-8
    if (charset_name) *charset_name = "UTF-8";
...
// --- CalcCRC -----------------------------------------------------------------
CalcCRC CalcCRC::CalcCRC_CRC16_CCITT(
    true, true, 0x1021);  // 0001 0000 0010 0001 (16, 12, 5, 0)
CalcCRC CalcCRC::CalcCRC_CRC16_IBM(
    true, false, 0x8005);  // 1000 0000 0000 0101 (16, 15, 2, 0)
CalcCRC CalcCRC::CalcCRC_FIRE_CODE(
    false, false,
    0x782F);  // 0111 1000 0010 1111 (16, 14, 13, 12, 11, 5, 3, 2, 1, 0)
...
bool BitReader::GetBits(int& result, size_t count) {
  ...
  while (count) {
    ...
    result_value |=
        (*data & (0xFF >> data_bits)) >> (8 - data_bits - copy_bits);
...
void BitWriter::AddBits(int data_new, size_t count) {
  while (count) {
    ...
    if (byte_bits == 0) data.push_back(0x00);
...
void BitWriter::AddBytes(const uint8_t* data, size_t len) {
  for (size_t i = 0; i < len; i++) AddBits(data[i], 8);
}
...
const dab_channels_t dab_channels{
    ...
    {"11A", 216928}, {"11N", 217088}, {"11B", 218640}, {"11C", 220352},
    {"11D", 222064},

    {"12A", 223936}, {"12N", 224096}, {"12B", 225648}, {"12C", 227360},
    {"12D", 229072},
