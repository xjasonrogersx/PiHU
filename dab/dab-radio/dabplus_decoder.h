...
class DabPlusServiceComponentDecoder {
 public:
  ...
  //  std::vector<uint8_t>&)>; virtual std::shared_ptr<PAD_DATA_CALLBACK>
  //  registerPadDataCallback(PAD_DATA_CALLBACK cb);

  // using AUDIO_COMPONENT_DATA_CALLBACK = std::function<void(const
  // std::vector<uint8_t>&, int, int, int, bool, bool)>; virtual
  // std::shared_ptr<AUDIO_COMPONENT_DATA_CALLBACK>
  // registerAudioDataCallback(AUDIO_COMPONENT_DATA_CALLBACK cb);
 ...
 private:
  ...
  // CallbackDispatcher<std::function<void (const std::vector<uint8_t>&)>>
  // m_padDataDispatcher; CallbackDispatcher<AUDIO_COMPONENT_DATA_CALLBACK>
  // m_audioDataDispatcher;
