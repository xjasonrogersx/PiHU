...
class RaonTunerInput {
 public:
  ...
  void setFicObserver(FicObserver* fic_observer) {
    m_fic_observer = fic_observer;
  };
  void setMscObserver(MscObserver* msc_observer) {
    m_msc_observer = msc_observer;
  };
 ...
 private:
  ...
  constexpr static uint8_t g_abAdcClkSynTbl[4][7] = {
      {0x08, 0x01, 0x13, 0x25, 0x06, 0x7D,
       0xB4},  // Based 24.576MHz,	8MHz	   External Clock21
      {0x06, 0x01, 0x0F, 0x27, 0x07, 0x60,
       0xB8},  // Based 24.576MHz,	8.192MHz   External Clock20
      {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
       0xFF},  // Based 24.576MHz, 9MHz       Unsupported Clock
      {0x0C, 0x01, 0x1F, 0x27, 0x06, 0xE1,
       0xB8}  // Based 24.576MHz,	9.6MHz	   External Clock22
  };
  ...
  static constexpr uint8_t MSC1_INTR_BITS =
      (MSC1_E_INT | MSC1_E_UNDER_FLOW | MSC1_E_OVER_FLOW);
  static constexpr uint8_t MSC0_INTR_BITS =
      (MSC0_E_INT | MSC0_E_UNDER_FLOW | MSC0_E_OVER_FLOW);

  static constexpr uint8_t INT_E_UCLRL =
      0x35;  /// [2]MSC1 int clear [1]MSC0 int clear [0]FIC int clear
  static constexpr uint8_t INT_E_UCLRH = 0x36;  /// [6]OFDM TII done clear

  static constexpr uint8_t INT_E_STATL =
      0x33;  /// [7]OFDM Lock status [6]MSC1 overrun [5]MSC1 underrun [4]MSC1
             /// int [3]MSC0 overrun [2]MSC0 underrun [1]MSC0 int [0]FIC int
  static constexpr uint8_t INT_E_STATH =
      0x34;  /// [7]OFDM NIS [6]OFDM TII [5]OFDM scan [4]OFDM window position
             /// [3]OFDM unlock [2]FEC re-configuration [1]FEC CIF end [0]FEC
             /// soft reset

  static constexpr uint8_t RTV_DAB_OFDM_LOCK_MASK = 0x01;
  static constexpr uint8_t RTV_DAB_FEC_LOCK_MASK = 0x02;
  static constexpr uint8_t RTV_DAB_CHANNEL_LOCK_OK =
      (RTV_DAB_OFDM_LOCK_MASK | RTV_DAB_FEC_LOCK_MASK);
