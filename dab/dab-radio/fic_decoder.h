...
struct FIG1_HEADER {
  ...
  FIG1_HEADER(uint8_t data)
      : charset(data >> 4), oe(data & 0x08), extension(data & 0x07) {}
};
...
struct FIC_LABEL {
  ...
  bool operator==(const FIC_LABEL& fic_label) const {
    return charset == fic_label.charset &&
           !memcmp(label, fic_label.label, sizeof(label)) &&
...
struct FIC_SUBCHANNEL {
  ...
  bool operator==(const FIC_SUBCHANNEL& fic_subchannel) const {
    return start == fic_subchannel.start && size == fic_subchannel.size &&
           pl == fic_subchannel.pl && bitrate == fic_subchannel.bitrate &&
...
struct FIC_ASW_CLUSTER {
  ...
  bool operator==(const FIC_ASW_CLUSTER& fic_asw_cluster) const {
    return asw_flags == fic_asw_cluster.asw_flags &&
           subchid == fic_asw_cluster.subchid;
  }
  bool operator!=(const FIC_ASW_CLUSTER& fic_asw_cluster) const {
...
struct FIC_DAB_DT {
  ...
  FIC_DAB_DT() : ms(ms_none) { dt.tm_year = none; }

  bool operator==(const FIC_DAB_DT& fic_dab_dt) const {
    return ms == fic_dab_dt.ms && dt.tm_sec == fic_dab_dt.dt.tm_sec &&
...
struct FIC_ENSEMBLE {
  ...
  bool operator==(const FIC_ENSEMBLE& ensemble) const {
    return eid == ensemble.eid && label == ensemble.label &&
           ecc == ensemble.ecc && lto == ensemble.lto &&
...
struct FIC_SERVICE {
  ...
  bool HasNoPriCompSubchid() const {
...
struct LISTED_SERVICE {
  ...
  bool operator<(const LISTED_SERVICE& service) const {
    ...
    if (sid != service.sid) return sid < service.sid;
...
class FICDecoder {
 private:
  ...
  void UpdateListedService(const FIC_SERVICE& service, int scids,
                           bool multi_comps);
 ...
 public:
  FICDecoder(FICDecoderObserver* observer, bool disable_dyn_msgs)
      : observer(observer), disable_dyn_msgs(disable_dyn_msgs) {}
  ...
  static std::string ConvertDateTimeToString(FIC_DAB_DT utc_dt, const int lto,
                                             bool output_ms);
  static std::string ConvertPTYToString(const int value,
                                        const int inter_table_id);
  static std::string ConvertASuTypeToString(const int value);
  static std::string DeriveShortLabelUTF8(const std::string& long_label,
                                          uint16_t short_label_mask);
};
